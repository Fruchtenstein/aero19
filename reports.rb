...
r = `litecli 2019.db -te "select runnername,date,time/distance s,distance, log.runnerid from log,runners where date>'#{bow}' and date<'#{eow}' and log.runnerid=runners.runnerid order by s limit 5"`
puts r
r = `litecli 2019.db -te "select runnername,date,time/distance s,distance, log.runnerid from log,runners where date>'#{bow}' and date<'#{eow}' and log.runnerid=runners.runnerid and sex=0 order by s limit 5"`
puts r
puts "самая быстрая неделя"
r = `litecli 2019.db -te "SELECT l.runnerid, runnername, t/d pace, strftime('%M:%S',t/d,'unixepoch'), teamname FROM (SELECT runnerid, SUM(time) t, SUM(distance) d FROM log WHERE date>'#{bow}' AND date<'#{eow}' AND time>0 GROUP BY runnerid) l, runners, teams WHERE runners.runnerid=l.runnerid AND teams.teamid=runners.teamid ORDER BY pace LIMIT 5"`
puts r
